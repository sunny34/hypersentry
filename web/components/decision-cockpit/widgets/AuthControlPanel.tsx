    ...
    return (
        <div className="p-4 border border-cyan-500/20 bg-cyan-950/10 rounded flex flex-col gap-3">
            <div className="flex items-center justify-between">
                ...
                <span className={`text-[10px] font-bold uppercase ${isAuthenticated ? 'text-green-400' : 'text-yellow-400'}`}>
            ...
            <div className="text-[9px] text-gray-500 flex items-center justify-between">
                <span>Wallet: <span className={isConnected ? 'text-green-400' : 'text-gray-400'}>{isConnected ? 'Connected' : 'Disconnected'}</span></span>
                <span>Agent: <span className={isAgentActive ? 'text-green-400' : 'text-yellow-400'}>{isAgentActive ? 'Active' : 'Inactive'}</span></span>
            </div>
