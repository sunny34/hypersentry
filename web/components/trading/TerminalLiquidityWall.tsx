...
export default function TerminalLiquidityWall({
    ...
    return (
        <div className="h-full bg-gradient-to-b from-[#0a0505] to-[#050505] border border-red-500/20 rounded-xl overflow-hidden flex flex-col">
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-2.5 bg-gradient-to-r from-red-500/10 to-transparent border-b border-red-500/20">
                <div className="flex items-center gap-2">
                    <div className="relative group">
                        ...
                        {/* Tooltip */}
                        <div className="absolute left-0 top-full mt-2 w-64 p-2 bg-black/95 border border-white/20 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                            <div className="text-[9px] text-gray-300 leading-relaxed">
                                <strong className="text-white">What is Liquidity Wall?</strong>
                                <br /><br />
                                Large order clusters in the order book that act as <span className="text-emerald-400">support</span> (buy walls) or <span className="text-red-400">resistance</span> (sell walls).
                                <br /><br />
                    ...
                    <div>
                        <span className="text-[10px] font-black uppercase tracking-wider text-white flex items-center gap-1">
                            ...
                            <span className="text-[7px] bg-red-500/20 text-red-400 px-1.5 rounded font-mono">LIVE</span>
                        </span>
                    </div>
                </div>
                <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold ${impulse === 'bullish' ? 'bg-emerald-500/20 text-emerald-400' :
                        impulse === 'bearish' ? 'bg-red-500/20 text-red-400' :
                ...
                >
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                            ...
                            <span className={`text-[9px] font-black uppercase ${primaryWall.side === 'support' ? 'text-emerald-400' : 'text-red-400'
                                }`}>
                    ...
                    <div className="flex items-center justify-between text-[10px]">
                        <span className={`font-mono font-bold ${primaryWall.side === 'support' ? 'text-emerald-300' : 'text-red-300'
                            }`}>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                            <div className="flex items-center gap-1.5 mb-2 px-1">
                                ...
                                <span className="text-[8px] font-black uppercase text-red-400 tracking-wider">Resistance</span>
                            </div>
                            <div className="space-y-1.5">
                                {resistances.length > 0 ? resistances.map((level, i) => (
                                    <div
                                        key={level.price}
                                        className="bg-red-500/5 border border-red-500/10 hover:border-red-500/30 rounded-lg p-2 cursor-pointer transition-all"
                                    ...
                                    >
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className={`text-[7px] font-black uppercase px-1 rounded ${level.strength === 'critical' ? 'bg-red-500 text-white animate-pulse' :
                                                level.strength === 'major' ? 'bg-red-500/30 text-red-300' :
                        ...
                        <div>
                            <div className="flex items-center gap-1.5 mb-2 px-1">
                                ...
                                <span className="text-[8px] font-black uppercase text-emerald-400 tracking-wider">Support</span>
                            </div>
                            <div className="space-y-1.5">
                                {supports.length > 0 ? supports.map((level, i) => (
                                    <div
                                        key={level.price}
                                        className="bg-emerald-500/5 border border-emerald-500/10 hover:border-emerald-500/30 rounded-lg p-2 cursor-pointer transition-all"
                                        onClick={() => onPriceClick?.(level.price)}
                                    >
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className={`text-[7px] font-black uppercase px-1 rounded ${level.strength === 'critical' ? 'bg-emerald-500 text-black animate-pulse' :
                                                level.strength === 'major' ? 'bg-emerald-500/30 text-emerald-300' :
            ...
            {/* Footer Bias */}
            <div className="px-3 py-2 border-t border-white/5 bg-black/40 flex items-center justify-between">
                <span className="text-[8px] text-gray-600">Market Bias:</span>
                <span className={`text-[9px] font-black uppercase ${marketBias === 'bullish' ? 'text-emerald-400' :
                    marketBias === 'bearish' ? 'text-red-400' :
                        'text-gray-400'
                    }`}>
                    {marketBias}
                </span>
