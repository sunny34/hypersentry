            ...
            {liqLevels.map((level, i) => {
                ...
                if (mode === 'full') {
                    ...
                    const bgStyle = isShort
                        ? `linear-gradient(90deg, rgba(234, 88, 12, 0) 0%, rgba(234, 88, 12, ${0.1 + (level.intensity * 0.2)}) 50%, rgba(234, 88, 12, ${0.3 + (level.intensity * 0.3)}) 100%)`
                        : `linear-gradient(90deg, rgba(220, 38, 38, 0) 0%, rgba(220, 38, 38, ${0.1 + (level.intensity * 0.2)}) 50%, rgba(220, 38, 38, ${0.3 + (level.intensity * 0.3)}) 100%)`;
                    ...
                    return (
                        ...
                        >
                            <div
                                className="w-full h-[24px] border-b border-white/5"
                                style={{
                                    background: bgStyle,
                            ...
                            <div className="absolute right-16 flex items-center gap-2 opacity-80">
                                <span className={`text-[10px] font-bold ${isShort ? 'text-orange-400' : 'text-red-400'}`}>{level.leverage}</span>
                ...
                const barColor = isShort
                    ? 'bg-gradient-to-l from-orange-500 to-orange-600/80 border-orange-400/30'
                    : 'bg-gradient-to-l from-red-500 to-red-600/80 border-red-400/30';

                const textColor = isShort ? 'text-orange-100' : 'text-red-100';
                const badgeColor = isShort ? 'bg-orange-950/40 text-orange-200' : 'bg-red-950/40 text-red-200';
                ...
                return (
                    ...
                    >
                        ...
                        >
                            ...
                            {/* Glow Effect */}
                            <div className={`absolute inset-0 rounded-l-full bg-gradient-to-r ${isShort ? 'from-orange-400/20' : 'from-red-400/20'} to-transparent blur-md -z-10`} />
                        </div>
                    </div>
                );
            })}
