...
export default function TwapIntelligence({ symbol, compact = false }: TwapIntelligenceProps) {
    ...
    return (
        <div className="h-full flex flex-col bg-[#050505] overflow-hidden">
            {/* Compact Header */}
            <div className="flex items-center justify-between px-3 py-2 bg-black/60 border-b border-white/5 shrink-0">
            ...
            {metrics && metrics.activeCount > 0 && (
                <div className={`px-3 py-2 border-b flex items-center justify-between ${metrics.sentiment === 'distributing'
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs font-bold uppercase ${metrics.sentiment === 'distributing' ? 'text-red-400' :
                                metrics.sentiment === 'accumulating' ? 'text-emerald-400' : 'text-gray-400'
                            }`}>
                    ...
                    <div className={`text-sm font-black font-mono ${metrics.netDelta > 0 ? 'text-emerald-300' : metrics.netDelta < 0 ? 'text-red-300' : 'text-gray-300'
                        }`}>
                ...
                {orders.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 gap-px bg-white/5">
                        ...
                        <div className="bg-[#050505]">
                            <div className="sticky top-0 bg-emerald-500/10 px-2 py-1.5 border-b border-emerald-500/20 flex items-center justify-between">
                                <div className="flex items-center gap-1.5">
                                    <TrendingUp className="w-3 h-3 text-emerald-400" />
                                    <span className="text-[9px] font-black text-emerald-400">BUYERS</span>
                                </div>
                                <span className="text-[8px] text-emerald-300/60 font-mono">
                            ...
                            <div className="p-1.5 space-y-1">
                                {buyOrders.length === 0 ? (
                                    ...
                                    buyOrders.map((order, i) => (
                                        <div
                                            key={order.hash || i}
                                            className="bg-emerald-500/5 border border-emerald-500/10 rounded px-2 py-1.5 hover:border-emerald-500/30 transition-all"
                                        >
                        ...
                        {/* SELLERS Column */}
                        <div className="bg-[#050505]">
                            <div className="sticky top-0 bg-red-500/10 px-2 py-1.5 border-b border-red-500/20 flex items-center justify-between">
                                <div className="flex items-center gap-1.5">
                                    <TrendingDown className="w-3 h-3 text-red-400" />
                                    <span className="text-[9px] font-black text-red-400">SELLERS</span>
                                </div>
                                <span className="text-[8px] text-red-300/60 font-mono">
                            ...
                            <div className="p-1.5 space-y-1">
                                {sellOrders.length === 0 ? (
                                    ...
                                    sellOrders.map((order, i) => (
                                        <div
                                            key={order.hash || i}
                                            className="bg-red-500/5 border border-red-500/10 rounded px-2 py-1.5 hover:border-red-500/30 transition-all"
                                        >
                                            <div className="flex items-center justify-between mb-1">
                                                <code className="text-[8px] text-gray-500 font-mono">
                                                    {formatAddress(order.address)}
