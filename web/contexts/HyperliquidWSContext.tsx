...
export function HyperliquidWSProvider({ children }: { children: React.ReactNode }) {
    ...
    const connect = useCallback(() => {
        ...
        console.log('🔌 Connecting to HL Global WS...');
        ...
        socket.onopen = () => {
            ...
            // Small delay for initial global subs to prevent race conditions
            setTimeout(() => {
        ...
        socket.onmessage = (event) => {
            try {
                ...
                if (channel === 'liquidations') {
                    console.log('✅ Received Liquidation Packet:', message);
                    const data = message.data;

                    // Hyperliquid can send an array of liquidations or a single one
                    const updates = Array.isArray(data) ? data : (data?.liquidations || [data]);
                    const newLiqs: any[] = [];

                    updates.forEach((item: any) => {
                        // HL events can be { liq: {...} } or {...} directly
                    ...
                    if (newLiqs.length > 0) {
                        console.log(`🔥 Adding ${newLiqs.length} new liquidations to feed`);
                        setLiquidations(prev => {
                            const combined = [...newLiqs, ...prev];
                            // Filter duplicates and keep last 100
                ...
                if (listeners.current.has('all')) {
                    listeners.current.get('all')?.forEach(cb => cb(message));
                }
            } catch (e) {
        ...
        socket.onclose = () => {
            console.log('❌ HL Global WS Disconnected');
        ...
        socket.onerror = (err) => {
            console.error('⚠️ HL Global WS Error');
            socket.close();
        };
    }, []);
    ...
    useEffect(() => {
        ...
        // Debug helper: Catch mock events
        ...
        window.addEventListener('mock-liquidation', handleMock);

        return () => {
            ws.current?.close();
            ws.current = null;
            window.removeEventListener('mock-liquidation', handleMock);
        };
    }, [connect]);
...
export function useHyperliquidWS() {
    ...
    if (!context) {
        throw new Error('useHyperliquidWS must be used within a HyperliquidWSProvider');
    }
    return context;
}
