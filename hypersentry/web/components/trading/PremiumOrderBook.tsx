...
export default function PremiumOrderBook({
    ...
    return (
        <div className="flex flex-col h-full w-full bg-[var(--background)] text-[var(--foreground)] text-[10px] select-none overflow-hidden">
            ...
            <div className="flex items-center justify-between px-3 py-2 bg-[var(--background)]/60 border-b border-[var(--glass-border)]">
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative">
                        ...
                        {showSettings && (
                            <div className="absolute top-full left-0 mt-1 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-lg shadow-2xl z-50 py-1 min-w-[60px]">
                                {[2, 3, 4, 5].map(p => (
                                    <button
                                        key={p}
                                        onClick={() => { setPrecision(p); setShowSettings(false); }}
                                        className={`block w-full text-left px-3 py-1.5 text-[9px] hover:bg-white/10 ${precision === p ? 'text-[var(--color-primary)]' : 'text-gray-400'}`}
                                    >
                                        {p} decimals
                                    </button>
                                ))}
            ...
            {/* Imbalance & CVD Bar */}
            <div className="px-3 py-1.5 bg-[var(--background)]/40 border-b border-[var(--glass-border)]">
                <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                        <span className="text-[8px] text-gray-600 font-bold">PRESSURE</span>
                        <span className={`text-[9px] font-black ${imbalance > 55 ? 'text-[var(--color-bullish)]' : imbalance < 45 ? 'text-[var(--color-bearish)]' : 'text-gray-400'}`}>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[9px] font-mono font-bold ${cvd >= 0 ? 'text-[var(--color-bullish)]' : 'text-[var(--color-bearish)]'}`}>
                            {cvd >= 0 ? '+' : ''}{cvd.toFixed(2)}
                        </span>
                        {recentDelta !== 0 && (
                            <span className={`text-[8px] font-mono animate-pulse ${recentDelta > 0 ? 'text-[var(--color-bullish)]' : 'text-[var(--color-bearish)]'}`}>
                                {recentDelta > 0 ? '↑' : '↓'}
                            </span>
                        )}
                    </div>
            ...
            <div className="flex-1 min-h-0 overflow-hidden">
                {view === 'depth' ? (
                    <div className="flex flex-col h-full">
                        {/* Column Headers */}
                        <div className="flex px-3 py-1.5 border-b border-[var(--glass-border)] text-[7px] font-black uppercase text-gray-600 tracking-widest bg-[var(--background)]/40 shrink-0">
