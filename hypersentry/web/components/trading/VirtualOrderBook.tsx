...
const OrderBookRow = memo(({ price, size, maxSize, side, precision, onSelect }: OrderBookRowProps) => {
    ...
    return (
        <div
            ...
            className="flex justify-between w-full text-[10px] font-mono items-center hover:bg-white/5 px-3 py-[1px] relative cursor-pointer group"
        >
            <div
                ...
                style={{ width: `${width}%` }}
            />
            <span className={`z-10 font-bold ${side === 'bid' ? 'text-emerald-400' : 'text-red-400'}`}>
                {px.toFixed(precision)}
            </span>
            <span className="text-gray-400 z-10 w-8 text-right group-hover:text-white transition-colors">
                {formatNumber(sz)}
            </span>
        </div>
    );
...
export const VirtualOrderBook = ({
    ...
    return (
        <div className="flex flex-col h-full w-full overflow-hidden">
            {/* Asks (Sells) - Top part, reversed so cheapest is at bottom */}
            <div className="flex-1 flex flex-col justify-end overflow-hidden">
                {[...safeAsks].reverse().map((ask, i) => (
                    <OrderBookRow
                        key={`ask-${ask.px}-${i}`}
                        price={ask.px}
                        size={ask.sz}
            ...
            <div className="py-2.5 my-1 bg-white/5 border-y border-white/5 flex flex-col items-center justify-center shrink-0">
