...
export default function DecisionNexus({ onBack, onSelectToken, onTabChange, selectedToken }: {
    ...
    useEffect(() => {
        if (selectedToken && selectedToken !== debateToken) {
            setDebateToken(selectedToken);
        }
    }, [selectedToken]);
    ...
    useEffect(() => {
        // Only auto-run if view is warroom AND (message list is empty OR token changed)
        if (view === 'warroom') {
            // If we just clicked warroom, run if empty. 
            // If we changed token, always run.
            // We can detect token change by checking if messages match current token? 
            // Simpler: Just rely on debateToken dependency, but we need to avoid double-run on mount.
            // The check for empty messages handles the "click back and forth" case.
            // But if I change token from BTC to ETH, messages are NOT empty (they are BTC messages).
            // So I need to clear messages when token changes?
            // Actually, the setDebateMessages([]) inside runDebate handles clearing.
            // I just need to know if I SHOULD run.
            // Let's rely on a simpler mechanic: 
            // If view is warroom, run. But we want to persist chat if we just switch tabs.
            // So: we need to separate "View Switch" from "Token Switch".
            // This is hard with just useEffect.
            // Let's clear messages when token changes explicitly in the UI handler?
            // No, let's keep it simple: If view is warroom, and we don't have messages, run.
            // If we change token, we should probably clear messages immediately anyway.
            // Let's add an explicit effect for token change.
        }
    }, [view]);

    // When token changes, clear messages so the main effect triggers? 
    // Or just trigger runDebate directly?
    useEffect(() => {
        if (view === 'warroom') {
            runDebate();
        } else {
    ...
    useEffect(() => {
        if (view === 'warroom' && debateMessages.length === 0) {
            runDebate();
        }
    }, [view]);
