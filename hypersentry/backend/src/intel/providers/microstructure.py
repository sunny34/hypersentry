...
class MicrostructureProvider(IntelProvider):
    ...
    def __init__(self):
        ...
        self.states = {} 
        asyncio.create_task(self._init_all_tokens())

    async def _init_all_tokens(self):
        """Initialize tracking for major tokens only (to avoid rate limits)."""
        # Only track majors by default. Others added dynamically on request.
        majors = ['BTC', 'ETH', 'SOL', 'HYPE', 'ARB', 'TIA', 'LINK', 'DOGE', 'AVAX', 'SUI']
        
        self.active_symbols = set(majors)
        logger.info(f"✅ Microstructure: Tracking {len(self.active_symbols)} major tokens.")
        
        for s in self.active_symbols:
            if s not in self.states:
                self.states[s] = self._create_empty_state()
                # Stagger backfills
                asyncio.create_task(self._backfill_data(s))
                await asyncio.sleep(1.0) # Stagger 1s per token to be safe
    ...
    async def _backfill_data(self, symbol: str):
        ...
        try:
            ...
            # Special case for BTC/ETH to ensure we always try Binance
            is_major = symbol in ['BTC', 'ETH', 'SOL', 'BNB', 'DOGE', 'AVAX'] 
    ...
    async def fetch_latest(self) -> List[Dict[str, Any]]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for sym in list(self.active_symbols):
                use_ext = self.states[sym].get("use_external", False)
                tasks.append(self._update_symbol_state(session, sym, market_state.get(sym), use_ext))
