...
### Stage 3 (Institutional Scale - CURRENT)

Full event-driven topology using a high-throughput message bus:

- **Ingest**: `IntelEngine` and `Aggregator` publish normalized events to Kafka.
- **Compute**: Service logic and Alpha Engine consume from Kafka.
- **Gateway**: FastAPI (Gunicorn) serves clients from Redis snapshots + real-time Kafka fanout.
- **Deployment**: Managed via Coolify (Self-hosted PaaS) on a dedicated VPS.
...
- `staging`: Production-mirror infra, session keys limited to testnet
- `production`: Hardened auth, SSL termination via Traefik (Coolify), Gunicorn workers

## 5) Process Startup Commands (Production)

### Backend (Gunicorn + Uvicorn Workers)

The production Docker image uses `gunicorn` to manage multiple worker processes for high availability:

```bash
gunicorn main:app -k uvicorn.workers.UvicornWorker -b 0.0.0.0:8000 --workers 4 --timeout 120
```
...
### Option B: Self-hosted via Coolify (Recommended)

1. Connect your GitHub repository to Coolify.
2. Create a new **Docker Compose** service.
3. Use `docker-compose.prod.yml` as the source configuration.
4. Set the following domains:
   - `web.yourdomain.com` -> Service: `web` (Port 3000)
   - `api.yourdomain.com` -> Service: `api` (Port 8000)
5. Add mandatory Environment Variables (JWT, API keys, etc).
6. Coolify/Traefik will handle the **SSL Certificates** and WebSocket upgrades automatically.
