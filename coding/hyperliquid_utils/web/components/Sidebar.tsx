...
export default function Sidebar({ view, setView, onImport, onAdd, currentView, onViewChange, isMobileOpen = false, onMobileClose }: SidebarProps) {
    ...
    return (
        <>
            ...
            {/* Sidebar Container */}
            <div className={`
                fixed inset-y-0 left-0 z-50 bg-black/95 backdrop-blur-xl border-r border-gray-800 flex flex-col transition-transform duration-300 ease-in-out
                ${isCollapsed ? 'lg:w-20' : 'lg:w-64'}
                w-64 
                ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
            `}>
                {/* Brand */}
                <div className={`p-6 border-b border-gray-800 flex items-center justify-between transition-all shrink-0`}>
                    <div className={`flex items-center gap-3 ${isCollapsed ? 'lg:justify-center lg:w-full' : ''}`}>
                        ...
                        {(!isCollapsed || isMobileOpen) && <h1 className="text-lg font-bold tracking-tight whitespace-nowrap block lg:group-hover:block">
                            Hyperliquid <span className="text-green-500">Sentry</span>
                        </h1>}
                    </div>

                    {/* Desktop Collapse Toggle */}
                    {!isCollapsed && (
                        <button onClick={toggle} className="p-1 hover:bg-gray-800 rounded-lg text-gray-400 transition hidden lg:block">
                            <ChevronLeft className="w-5 h-5" />
                        </button>
                    )}

                    {/* Mobile Close Button */}
                    <button onClick={onMobileClose} className="p-1 hover:bg-gray-800 rounded-lg text-gray-400 transition lg:hidden">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                {isCollapsed && (
                    <div className="w-full justify-center py-2 border-b border-gray-800/50 hidden lg:flex">
