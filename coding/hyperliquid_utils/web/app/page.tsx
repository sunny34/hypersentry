...
import { Activity, Shield, Zap, Trash2, Plus, Upload, X, TrendingUp, Eye, AlertTriangle, Sparkles, BarChart3 } from 'lucide-react';
...
export default function Home() {
  ...
  const fetchStats = async () => {
    try {
      ...
      setStats(res.data);
      const res2 = await axios.get(`${API_URL}/wallets`);
      setWallets(res2.data.wallets || []);
      const res3 = await axios.get(`${API_URL}/twap`);
      setTwaps(res3.data.tokens || []);
      const res4 = await axios.get(`${API_URL}/twap/active`);
  ...
  const deleteWallet = async (addr: string) => {
    await axios.delete(`${API_URL}/wallets/${addr}`);
    fetchStats();
  };
  ...
  const addWallet = async (e: any) => {
    ...
    await axios.post(`${API_URL}/wallets/add`, { address: addr, label: label, active_trading: isTrading });
    setShowAdd(false);
    fetchStats();
  };
  ...
  const addTwap = async (e: any) => {
    ...
    await axios.post(`${API_URL}/twap/add`, { token });
    e.target.reset();
    fetchStats();
  };

  const removeTwap = async (token: string) => {
    await axios.delete(`${API_URL}/twap/${token}`);
    fetchStats();
  };
  ...
  const updateMinSize = async (e: any) => {
    ...
    await axios.post(`${API_URL}/twap/config`, { min_size: size });
    fetchStats();
  };
